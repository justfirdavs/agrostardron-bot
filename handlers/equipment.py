"""Wizard for a drone's leader/pilots to enter the real generator / battery /
vehicle data for their drone (replaces the placeholder values from the
...
async def equipment_menu(message: Message, state: FSMContext):
    ...
    if not user or user["role"] not in ("leader", "pilot"):
        await message.answer("Этот раздел доступен руководителю и пилотам команды.")
        return
...
async def gen_serial(message: Message, state: FSMContext):
    await state.update_data(gen_serial=message.text.strip().upper())
...
async def bat1_serial(message: Message, state: FSMContext):
    await state.update_data(bat1_serial=message.text.strip().upper())
...
async def bat2_serial(message: Message, state: FSMContext):
    await state.update_data(bat2_serial=message.text.strip().upper())
...
async def bat3_serial(message: Message, state: FSMContext):
    await state.update_data(bat3_serial=message.text.strip().upper())
...
async def veh_vin(message: Message, state: FSMContext):
    ...
    await state.update_data(veh_vin=None if text == "-" else text.upper())
...
async def veh_last_oil(message: Message, state: FSMContext, bot: Bot):
    value = _parse_number(message.text)
    if value is None:
        await message.answer("Нужно число. Если не меняли — отправьте 0.")
        return
    ...
    battery_ids = await db.replace_batteries_for_drone(
        drone_serial,
        [
    ...
    await notify.check_generator(bot, data["gen_serial"])
    for bid in battery_ids:
        await notify.check_battery(bot, bid)
    await notify.check_vehicle(bot, data["veh_plate"])
